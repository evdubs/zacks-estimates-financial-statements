...
(define (download-day date)
  (make-directory* (string-append "/var/tmp/zacks/dividend-calendar/" (~t (today) "yyyy-MM-dd")))
  (call-with-output-file* (string-append "/var/tmp/zacks/dividend-calendar/" (~t (today) "yyyy-MM-dd") "/"
    ...
    (λ (out)
      (with-handlers ([exn:fail?
        ...
        (~> (string-append "https://www.zacks.com/includes/classes/z2_class_calendarfunctions_data.php"
                           "?calltype=eventscal&type=5&date="
...
(command-line
 #:program "racket dividend-calendar-extract.rkt"
 #:once-each
 [("-e" "--end-date") ed
