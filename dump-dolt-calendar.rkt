...
(define base-folder (make-parameter "/var/tmp/dolt/calendar-estimates-statements"))
...
(command-line
 ...
 [("-b" "--base-folder") folder
                         "Base dolt folder. Defaults to /var/tmp/dolt/calendar-estimates-statements"
                         (base-folder folder)]
 [("-d" "--date") date
...
(system (string-append "cd " (base-folder) "; /usr/local/bin/dolt add earnings_calendar; "
                       "/usr/local/bin/dolt commit -m 'earnings_calendar " (as-of-date) " update'; /usr/local/bin/dolt push"))
