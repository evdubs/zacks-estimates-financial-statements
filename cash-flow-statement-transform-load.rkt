...
(display "zacks cash flow statement folder [/var/tmp/zacks/cash-flow-statement]: ")
(flush-output)
(define cash-flow-statement-base-folder
  (let ([cash-flow-statement-base-folder-input (read-line)])
    (if (equal? "" cash-flow-statement-base-folder-input) "/var/tmp/zacks/cash-flow-statement"
        cash-flow-statement-base-folder-input)))

(display (string-append "cash flow statement folder date [" (date->string (current-date) "~1") "]: "))
(flush-output)
(define folder-date
...
(parameterize ([current-directory (string-append cash-flow-statement-base-folder "/" (date->string folder-date "~1") "/")])
  (for ([p (sequence-filter (λ (p) (string-contains? (path->string p) ".cash-flow-statement.html")) (in-directory))])
    (let ([file-name (string-append cash-flow-statement-base-folder "/" (date->string folder-date "~1") "/" (path->string p))]
